...
describe("ManipulateGeometryBehavior", () => {
	describe("constructor", () => {
		it("constructs", () => {
			new ManipulateGeometryBehavior(MockBehaviorConfig("test"), {
				validate: undefined,
				onSuccess: jest.fn(),
			});
		});
	...
	describe("api", () => {
		describe("coordinateAtIndexIsIdentical", () => {
			it("works for Polygon/LineString and throws for invalid Point index", () => {
				...
				const behavior = new ManipulateGeometryBehavior(config, {
					validate: undefined,
					onSuccess: jest.fn(),
				});
		...
		describe("createPolygonGeometry", () => {
			it("creates a feature and calls onSuccess", () => {
				...
				const created = behavior.createPolygonGeometry({
					coordinates: coords,
		...
		describe("updatePolygon", () => {
			...
			it("applies coordinate mutations (insert/update/delete/tail insert)", () => {
				...
				const mutations: CoordinateMutation[] = [
					{ type: Mutations.INSERT, index: 2, coordinate: [0.5, 0.5] }, // before original index 2
					...
					{ type: Mutations.INSERT, index: 5, coordinate: [9, 9] }, // tail insert (append)
				];
			...
			it("supports negative indices for inserts", () => {
				...
				const updated = behavior.updatePolygon({
					...
					updateType: UpdateTypes.Commit,
					coordinateMutations: [
						{ type: Mutations.INSERT, index: -1, coordinate: [7, 7] },
					],
				});
		...
		describe("epsilonOffset", () => {
			it("returns >= 1e-6 for high precision and equals epsilon for lower precision", () => {
				...
				const behaviorHigh = new ManipulateGeometryBehavior(configHigh, {
					validate: undefined,
					onSuccess: jest.fn(),
				});
				...
				const behaviorLow = new ManipulateGeometryBehavior(configLow, {
					validate: undefined,
					onSuccess: jest.fn(),
				});
