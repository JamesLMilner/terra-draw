...
class Arc<Properties extends JSONObject> {
    ...
    toJSON(): Feature<LineString> | null {
        if (this.geometries.length === 1) {
            ...
            if (
                coords[0][0] && !isNaN(coords[0][0]) &&
                coords[0][1] && !isNaN(coords[0][1])
            ) {
                return {
...
class GreatCircleLine<Properties extends JSONObject> {
    ...
    constructor(start: Position, end: Position, properties?: Properties) {
        ...
        if (this.g === Math.PI) {
            throw new Error(`it appears ${start} and ${end} are 'antipodal', e.g diametrically opposite, thus there is no single route but rather infinite`);
...
export function greatCircleLine<Properties extends JSONObject>({ start, end, options }: {
    start: Position,
    end: Position,
    options?: {
    ...
    const { properties = {}, numberOfPoints = 100, offset = 10, coordinatePrecision = 9 } = opts;
    const circle = new GreatCircleLine(start, end, properties);
    const line = circle.arc(numberOfPoints, { offset: offset, coordinatePrecision });
