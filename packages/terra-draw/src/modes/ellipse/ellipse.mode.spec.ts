...
describe("TerraDrawEllipseMode", () => {
	...
	describe("showCoordinatePoints", () => {
		const setup = (
			enabled = true,
			projection: "globe" | "web-mercator" = "web-mercator",
		) => {
			const mode = new TerraDrawEllipseMode({
				showCoordinatePoints: enabled,
				projection,
			});
			...
			const expectMatchingPoints = (count = 64) => {
		...
		it.each(["globe", "web-mercator"] as const)(
			"keeps points aligned while drawing and after finishing in %s",
			(projection) => {
				const { mode, expectMatchingPoints } = setup(true, projection);
		...
		it("supports drag drawing and changes to the segment count", () => {
			const { mode, expectMatchingPoints } = setup();
			mode.updateOptions({ drawInteraction: "click-drag", segments: 12 });
			mode.onDragStart(MockCursorEvent({ lng: 0, lat: 0 }), jest.fn());
			mode.onDrag(MockCursorEvent({ lng: 2, lat: 1 }), jest.fn());
			expectMatchingPoints(12);
			mode.updateOptions({ segments: 24 });
			mode.onDragEnd(MockCursorEvent({ lng: 3, lat: 1 }), jest.fn());
			expectMatchingPoints(24);
		});

		it("toggles points for existing ellipses without duplicating them", () => {
		...
		it("creates and updates points for externally added and updated ellipses", () => {
