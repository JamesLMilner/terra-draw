...
A GitHub codespace can be used to create a development environment. The [Live Server](https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer) extension can be used to view a locally served instance of Terra-Draw. It can be installed from the extensions tab (accessed with `(Ctrl + Shift + x)`).

After the environment is set up, follow the instructions in [Building the Project](#Building-the-Project).
