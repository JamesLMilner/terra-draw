...
### Rectangle Drawing Interaction Methods

`TerraDrawRectangleMode` supports two distinct drawing interaction patterns:

#### Default Click-Move-Click Method

The standard drawing approach follows this sequence:
1. **First click** - Sets the starting corner coordinate
2. **Mouse movement** - Dynamically adjusts the rectangle size as you move the pointer
3. **Second click** - Finalizes the rectangle and completes the drawing

#### Click-and-Drag Method

Alternatively, you can enable a click-and-drag interaction pattern:
1. **Click and hold** - Sets the starting corner coordinate
2. **Drag** - Dynamically adjusts the rectangle size while dragging
3. **Release** - Finalizes the rectangle and completes the drawing

#### Configuration

The `drawInteraction` option controls which drawing methods are enabled. The following values are supported:

- `"click-move"` - Enables only the click-move-click method (default)
- `"click-drag"` - Enables only the click-and-drag method
- `"click-move-or-drag"` - Enables both drawing methods simultaneously

If not specified, `drawInteraction` defaults to `"click-move"`.

**Example:** Configuring rectangle mode to use click-and-drag only:
