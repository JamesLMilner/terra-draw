...
If you enable undo/redo, Terra Draw emits a `history` event whenever the undo/redo stacks change.

You can use this to enable or disable custom UI controls (for example, Undo and Redo buttons):
...
const draw = new TerraDraw({
  ...
  undoRedo: {
    drawingLevel: new TerraDrawDrawingUndoRedo(), // Optional
    sessionLevel: new TerraDrawSessionUndoRedo(),  // Optional
    keyboardShortcuts: new TerraDrawUndoRedoKeyboardShortcuts(),  // Optional
  },
});
...
`drawingLevel` and `sessionLevel` are optional and work independently, so you can configure either one on its own or both together.

If you want to disable drawing-level undo/redo, omit `drawingLevel` from `undoRedo`.
