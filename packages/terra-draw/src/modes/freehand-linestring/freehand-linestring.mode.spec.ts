	...
	describe("showCoordinatePoints", () => {
		const setup = (enabled = true) => {
			...
			const expectMatchingPoints = (
				count = (lineString().geometry as LineString).coordinates.length,
			) => {
			...
			return { mode, config, points, lineString, expectMatchingPoints };
		};
		...
		it("tracks sampled coordinates during drag drawing and after finishing", () => {
			...
			expectMatchingPoints(2);
			mode.onDrag(MockCursorEvent({ lng: 10, lat: 10 }), jest.fn());
			expectMatchingPoints(3);
			expect(points()[2].geometry.coordinates).toEqual([10, 10]);
		...
		it("toggles points for existing freehand linestrings without duplicating them", () => {
		...
		it("creates and updates points for externally added and updated freehand linestrings", () => {
			const { mode, config, lineString, expectMatchingPoints } = setup(false);
			mode.onClick(MockCursorEvent({ lng: 0, lat: 0 }));
			mode.onMouseMove(MockCursorEvent({ lng: 2, lat: 1 }));
			mode.onMouseMove(MockCursorEvent({ lng: 2, lat: 3 }));
			mode.onKeyUp(MockKeyboardEvent({ key: "Enter" }));
			...
			mode.updateOptions({ showCoordinatePoints: true });
			const coordinates = (feature.geometry as LineString).coordinates.map(
				([lng, lat]) => [lng + 1, lat + 1],
			);
			config.store.updateGeometry([
				{ id: feature.id!, geometry: { type: "LineString", coordinates } },
			]);
			mode.afterFeatureUpdated(lineString());
			expectMatchingPoints();
		});
