import { getPixelDistance } from "./get-pixel-distance";
...
describe("Geometry", () => {
  describe("getPixelDistance", () => {
    it("vertical distance", () => {
      const result = getPixelDistance({ x: 0, y: 0 }, { x: 0, y: 1 });
      expect(result).toBe(1);
    });
    it("horizontal distance", () => {
      const result = getPixelDistance({ x: 0, y: 0 }, { x: 1, y: 0 });
      expect(result).toBe(1);
    });
    it("diagonal distance", () => {
      const result = getPixelDistance({ x: 0, y: 0 }, { x: 1, y: 1 });
