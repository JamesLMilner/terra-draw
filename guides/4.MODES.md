...
The click-and-drag method is disabled by default. Enable it using the `clickAndDrag` option:

```typescript
new TerraDrawRectangleMode({ clickAndDrag: true })
```

> [!NOTE]
> When `clickAndDrag` is enabled, both interaction methods are available. Users can draw rectangles using either the traditional click-click approach or the click-and-drag method.
